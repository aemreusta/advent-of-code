...
def fuzzy_woozy(lines):
    ...
    for line in lines:
        ...
        if line[0][0] == line[1][0]: #on the same x axis, increases the y axis
            for i in range( min(line[0][1], line[1][1]) , min(line[0][1], line[1][1]) + abs(line[0][1] - line[1][1]) + 1 ):
                map[i][line[0][0]] += 1
    
        elif line[0][1] == line[1][1]: #on the same y axes, increases the x axes
            for i in range( min(line[0][0], line[1][0]) , min(line[0][0], line[1][0]) + abs(line[0][0] - line[1][0]) + 1):
                map[line[0][1]][i] += 1
...
#Part 1 Solution
lines = [i for i in ([list(map(int, x.split(","))) for x in line.split(" -> ")] for line in data) if i[0][0] == i[1][0] or i[0][1] == i[1][1]]
